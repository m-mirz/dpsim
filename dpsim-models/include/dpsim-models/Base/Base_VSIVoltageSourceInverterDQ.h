...
template <typename VarType> class VSIVoltageSourceInverterDQ {
private:
  /// Component logger
  Logger::Log mLogger;
...
public:
  explicit VSIVoltageSourceInverterDQ(Logger::Log Log,
                                      CPS::AttributeList::Ptr attributeList,
                                      Bool modelAsCurrentSource,
                                      Bool withInterfaceResistor)
      : mLogger(Log), mModelAsCurrentSource(modelAsCurrentSource),
        ...
        mSourceValue(attributeList->create<MatrixComp>("SourceValue",
                                                       MatrixComp::Zero(1, 1))),
        mSourceValue_dq(
            attributeList->create<Complex>("SourceValue_dq", Complex(0, 0))),
  ...
  /// Droop Controller
  void
  addDroopController(std::shared_ptr<Signal::DroopController> DroopController);
...
protected:
  ...
  ///
  void
  initializeFilterVariables(const Complex &interfaceVoltage,
